package co.com.gsdd.demo.mongo.controllers;
...
public class EmployeeController {
	...
	public Mono<ResponseEntity<Employee>> findByName(@PathVariable("name") String name) {
		return service.findByName(name).map(ResponseEntity::ok).defaultIfEmpty(ResponseEntity.notFound().build());
	}
	...
	public Mono<Employee> save(@Valid @RequestBody Employee employee) {
		return service.save(Employee.builder().employeeId(employee.getEmployeeId()).name(employee.getName()).build());
	}

	@PatchMapping("/{id}")
	public Mono<ResponseEntity<Employee>> patchName(@PathVariable("id") Long id, @Valid @RequestBody String name) {
	...
	public Mono<ResponseEntity<Void>> delete(@PathVariable("id") Long id) {
		return service.findById(id).flatMap(
				dbEmployee -> service.delete(dbEmployee).then(Mono.just(new ResponseEntity<Void>(HttpStatus.OK))))
